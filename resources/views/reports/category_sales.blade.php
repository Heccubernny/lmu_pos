        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm border border-slate-200 rounded-xl p-6">
                ...
                <div class="mb-8 bg-slate-50 p-5 rounded-xl border border-slate-200">
                    <form action="{{ route('admin.reports.category_sales') }}" method="GET" class="space-y-4">
                        ...
                        <div class="flex gap-2 justify-end pt-3 border-t border-slate-200/50">
                            ...
                            @if(!empty($storeId) || !empty($staffId) || !empty($category) || !empty($modePayment) || (isset($status) && $status !== 'completed') || (isset($period) && $period !== 'this_month') || !empty($startDate) || !empty($endDate) || !empty($startTime) || !empty($endTime))
                                <a href="{{ route('admin.reports.category_sales') }}" class="px-5 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 transition text-xs font-bold flex items-center">
                                    Clear
                                </a>
                            @endif
                        </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Quantity Sold</th>
                                <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Sales Revenue</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @forelse($categorySales as $cat)
                                <tr>
                                    <td class="px-6 py-4 text-sm font-medium text-slate-900">{{ $cat->category }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-500">{{ number_format($cat->total_qty) }}</td>
                                    <td class="px-6 py-4 text-sm font-bold text-slate-800">₦{{ number_format($cat->total_amount, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="px-6 py-12 text-sm text-slate-500 text-center">No category sales data found for the selected period.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
