...
<div class="py-8">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex items-center gap-2 text-xs text-slate-500 mb-6">
            ...
            <span class="text-slate-700 font-semibold">Edit Department</span>
        </nav>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="px-7 py-5 border-b border-slate-100 flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-base font-bold text-slate-800">Edit Department</h2>
                    <p class="text-xs text-slate-500 mt-0.5">Modify department properties and branch configuration.</p>
                </div>
            </div>

            {{-- Form --}}
            <form method="POST" action="{{ route('admin.departments.update', $department) }}" class="px-7 py-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="branch"
                        ...
                        <option value="Global Warehouse" {{ old('branch', $department->branch) == 'Global Warehouse' ? 'selected' : '' }}>Global Warehouse (Central)</option>
                        @foreach($stores as $store)
                            <option value="{{ $store->name }}" {{ old('branch', $department->branch) == $store->name ? 'selected' : '' }}>
                                {{ $store->name }}
                            </option>
                        @endforeach
                    </select>
